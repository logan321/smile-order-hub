...
const Admin = () => {
  ...
  const activeCount = subscribers.filter(s => s.status === 'active' || (s.status === 'trialing' && s.trial_ends_at && new Date(s.trial_ends_at) > new Date())).length;
  const trialCount = subscribers.filter(s => s.status === 'trialing').length;
  const expiredCount = subscribers.filter(s => ['canceled', 'past_due'].includes(s.status) || (s.status === 'trialing' && s.trial_ends_at && new Date(s.trial_ends_at) <= new Date())).length;
