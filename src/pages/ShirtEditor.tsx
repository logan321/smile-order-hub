  ...
  return (
    <div className="h-[100dvh] bg-background flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-h-0">
        ...
        <main className="flex flex-1" style={{height: 'calc(100vh - 60px)'}}>
          {/* Coluna 1: Navegação */}
          <nav id="left-sidebar" className="w-20 bg-white shadow-lg border-r border-gray-200 flex-shrink-0 flex flex-col items-center py-4 space-y-6">
            {[
              ...
              <button
                key={id}
                onClick={() => setActiveTab(activeTab === id ? null : (id as ToolbarTab))}
                className={`flex flex-col items-center gap-1 w-full ${activeTab === id ? 'text-[#FF5A00]' : 'text-gray-500 hover:text-gray-900'}`}
              >
          ...
          {activeTab && (
            <aside id="dynamicSidebar" className="w-80 bg-white shadow-lg border-r border-gray-200 flex-shrink-0 overflow-y-auto h-full p-6">
              ...
              {/* Painel Dinâmico: Conteúdo funcional restaurado */}
              {activeTab === 'stamps' && (
                <div className="animate-fade-in">
                  <p className="text-xs font-semibold text-muted-foreground uppercase mb-4 tracking-wider">Escolha uma estampa</p>
                  {stamps.length === 0 ? (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {stamps.map(s => (
                        <button key={s.id} onClick={() => addStamp(s)} className={`group rounded-xl border-2 p-1.5 transition-all bg-card hover:shadow-md ${appliedStamp?.id === s.id ? 'border-[#FF5A00] ring-2 ring-orange-100' : 'border-border/50 hover:border-[#FF5A00]/50'}`}>
                          ...
                          <p className="text-[10px] text-center text-muted-foreground mt-1.5 truncate font-bold">{s.name}</p>
                        </button>
                      ))}
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {patches.map(p => (
                        <button key={p.id} onClick={() => handlePatchClick(p)} className="group rounded-xl border-2 border-border/50 p-1.5 transition-all bg-card hover:border-[#FF5A00] hover:shadow-md">
                          ...
                          <p className="text-[9px] text-center text-muted-foreground mt-1 truncate font-bold">{p.name}</p>
                        </button>
                      ))}
                    </div>
                  )}
          ...
          <div className="flex-1 relative">
            ...
            <div className="absolute top-4 right-4 flex gap-2 z-50">
              <Button onClick={() => handleWhatsAppQuote()} className="bg-[#FF5A00] hover:bg-[#e65100]">Enviar Orçamento</Button>
              <Button onClick={handleDownload} variant="secondary">Salvar Simulação</Button>
            </div>
